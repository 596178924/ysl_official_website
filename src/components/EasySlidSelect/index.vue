<template>
	<div class="easy-slid-select">
		...
		<transition-group name="out-in" mode="out-in" tag='ul'  class="transition-ul">
			<li v-for="(item,index) in list" :key="index" v-show="currentIndex == index">
				<slot :item="item"></slot>
			</li>
		</transition-group>
	</div>
...
export default {
	...
	methods:{
		enterAlive(index) {
			let width = this.$refs['lis'][index].children[0].offsetWidth;
			let left =  this.$refs['lis'][index].children[0].offsetLeft;
			this.$refs['labelAlive'].style.width = width + 'px';
			this.$refs['labelAlive'].style.left = left + 'px';
		},
		leaveAlive(index) {
			if(index != this.currentIndex) {
				this.$refs['labelAlive'].style.left = this.$refs['lis'][this.currentIndex].children[0].offsetLeft + 'px';
				this.$refs['labelAlive'].style.width = this.$refs['lis'][this.currentIndex].children[0].offsetWidth + 'px';
			}
		}
	}
}
